...
import static com.github.pires.obd.reader.mqtt_odbii.AppConstants.DeviceIp;
import static com.github.pires.obd.reader.mqtt_odbii.AppConstants.PORT;
...
public class ObdGatewayService extends AbstractGatewayService {
    ...
    public static    long  OBD_INIT_TIMEOUT_VAL = 55;
    ...
    public void startService() throws IOException {
        ...
        final String remoteDevice = prefs.getString(ConfigActivity.BLUETOOTH_DEVICE_KEY, null);
    ...
    public void startServiceNetwork() throws IOException {
        Log.d(TAG, "Starting service..");


         String deviceIp = prefs.getString(ConfigActivity.NETWORK_DEVICE_IP, null);
         PORT = Integer.parseInt(prefs.getString(ConfigActivity.NETWORK_DEVICE_PORT, "35000"));
        ...
        }catch (Exception ex)
        {
            ...
            try {
                startObdConnectionOverNetwork(deviceIp);
            } catch (Exception e) {
                Log.e(
                        TAG,
                        "There was an error while establishing connection. -> "
    ...
    private void startObdConnectionOverNetwork(final String deviceIp) throws IOException {
        ...
        Thread connectThread=new Thread(new Runnable() {
            ...
            public void run() {
                ...
                try {
                    mSocket = new Socket();
                    device=deviceIp;
                    InetSocketAddress addr = new InetSocketAddress(device, PORT);
                    mSocket.connect(addr);
                } catch (Exception e2) {
                    Log.e(TAG, "There was an error while establishing Network connection. Stopping app..", e2);
                    stopService();
                    try {
                        throw new IOException();
                    } catch (IOException e) {
     ...
     */
                ...
                queueJob(new ObdCommandJob(new TimeoutCommand((int)OBD_INIT_TIMEOUT_VAL)));
    ...
    protected void executeQueueOverNetwork() throws InterruptedException {
        Log.d(TAG, "Executing queue..");
    ...
    public void stopServiceNetwork() {
        Log.d(TAG, "Stopping service..");
